...
def plot_qber_vs_distance(mu=0.1, distance_values=None, position_of_source="middle",
                   ...
                   weather_list=("very clear","clear","partly clear","hazy","foggy"), atmos_visibility_list=[40,30,20,10,5],is_decoherence=True, save_fig=False):
    """
    ...
    if channel_mode == "fso":
        ...
        if len(weather_list) > 0:
            for weather in weather_list:
                ...
                simulator = BBM92Simulator(
                    ...
                    channel_mode=channel_mode, weather=weather
                )
        ...
        if (weather_list is None or len(weather_list) == 0) and len(atmos_visibility_list) > 0:
            for visibility in atmos_visibility_list:
                ...
                simulator = BBM92Simulator(
                    ...
                    alice_distance=0,#will be updated in the loop
                    bob_distance=0,#will be updated in the loop
                    channel_mode=channel_mode, weather=None, atmos_visibility=visibility
                )
        ...
        plt.ylabel('QBER per pulse', fontsize=15)
        plt.title(f'Quantum Bit Error Rate vs Distance ({channel_mode.upper()} channel)',fontsize=15)
        plt.xticks(fontsize=15)
        plt.yticks(np.linspace(0,0.7,8),fontsize=15)
    ...
    if channel_mode == "fiber":
        ...
        for attenuation in attenuation_list:
            ...
            simulator = BBM92Simulator(
                ...
                alice_distance=0,#will be updated in the loop
                bob_distance=0,#will be updated in the loop
                channel_mode=channel_mode, bob_fiber_attenuation=attenuation, alice_fiber_attenuation=attenuation,is_decoherence=is_decoherence
            )
        ...
        plt.xlabel('Distance (km)', fontsize=15)
        plt.ylabel('QBER per pulse', fontsize=15)
        plt.title(f'Quantum Bit Error Rate vs Distance in {channel_mode.upper()} channel(in {deco} of decoherence)',fontsize=15)
        plt.axhline(0.11, color="orange", linestyle="--", alpha=0.7,label="QBER 11% thresold")
        plt.axhline(0.5, linestyle="--", alpha=0.6)
        plt.legend()
        plt.xticks(fontsize=15)
        ...
        if simulator.is_decoherence==True:
            ...
            plt.xlabel('Distance (km)', fontsize=15)
            plt.ylabel('QBER per pulse', fontsize=15)
            plt.title(f'Quantum Bit Error Rate vs Distance in {channel_mode.upper()} channel (in presence of decoherence)',fontsize=15)
            plt.legend( loc="upper right")
            plt.xticks(fontsize=15)
            plt.yticks(fontsize=15)
            plt.xlim(0,np.max(distance_values))
...
def plot_skr_vs_distance(mu=0.1, distance_values=None, position_of_source="middle",
                   ...
                   alice_channel_base_efficiency=1, bob_channel_base_efficiency=1,
                   alice_dark_count_rate=6.02e-6, bob_dark_count_rate=6.02e-6, channel_mode="fiber",fiber_attenuation_list=[0.18,0.21,0.24],
                   weather_list=("very clear","clear","partly clear","hazy","foggy"), atmos_visibility_list=[40,30,20,10,5], save_fig=False, log_scale=False):
    """
    ...
    if channel_mode == "fiber":
        ...
        for attenuation in attenuation_list:
            ...
            simulator = BBM92Simulator(
                ...
                alice_distance=0,#will be updated in the loop
                bob_distance=0,#will be updated in the loop
                channel_mode=channel_mode, bob_fiber_attenuation=attenuation, alice_fiber_attenuation=attenuation
            )
    ...
    plt.grid(False)
    plt.xlabel('Distance (km)', fontsize=15)
    plt.ylabel('Secret Key Rate (per second)', fontsize=15)
    plt.title(f'Secret Key Rate vs Distance ({channel_mode.upper()} channel)',fontsize=15)
